...
import { ChakraProvider, Box, Flex, VStack, HStack, Text, Icon, Button, Tabs, TabList, Tab, TabPanels, TabPanel, Grid, GridItem, Heading, useToast, IconButton, Menu, MenuButton, MenuList, MenuItem, Badge, Input } from '@chakra-ui/react';
...
        <Input className="form-control" type="text" value={login} onChange={e => setLogin(e.target.value)} onKeyPress={handleKeyPress} placeholder="Введите логин" />
      </Box>
      <Box w="full">
        <Text mb={2}>Пароль</Text>
        <Input className="form-control" type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyPress={handleKeyPress} placeholder="Введите пароль" />
      </Box>
...
  useEffect(() => {
    if (section === 'dashboard' && isAuthenticated && chartRef.current) {
...
      };
    }
  }, [bookings, users, section, isAuthenticated]);
...
          <canvas ref={chartRef} id="bookingsChart" height="100"></canvas>
        </Box>
      </Box>
    </Box>
  );
