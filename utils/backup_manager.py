...
class BackupConfig:
    ...
    KEEP_DAILY_BACKUPS = int(os.getenv("KEEP_DAILY_BACKUPS", "30"))       # 30 дней ежедневных
    KEEP_WEEKLY_BACKUPS = int(os.getenv("KEEP_WEEKLY_BACKUPS", "12"))     # 12 недель еженедельных
    KEEP_MONTHLY_BACKUPS = int(os.getenv("KEEP_MONTHLY_BACKUPS", "6"))    # 6 месяцев ежемесячных
...
class BackupMetadata:
    ...
    def remove_backup(self, backup_name: str):
        ...
        self._metadata["backups"] = [
            b for b in self._metadata["backups"] 
            if b.get("filename") != backup_name
        ]
        self._save_metadata()
...
class DatabaseBackupManager:
    ...
    def restore_backup(self, backup_filename: str, target_db: Optional[Path] = None) -> bool:
        """
    ...
    def cleanup_old_backups(self) -> Dict[str, int]:
        ...
        # Группируем бэкапы по типам/периодам
        now = datetime.utcnow()
        
        for backup in backups[:]:  # Копия списка для безопасного удаления
            try:
                ...
                # Логика ротации
                if age > timedelta(hours=self.config.KEEP_HOURLY_BACKUPS):
                    if age < timedelta(days=self.config.KEEP_DAILY_BACKUPS):
                        # Оставляем только один бэкап в день
                        daily_key = backup_date.strftime("%Y-%m-%d")
                        # Здесь можно добавить более сложную логику
                        should_delete = False
                    elif age < timedelta(weeks=self.config.KEEP_WEEKLY_BACKUPS):
                        # Оставляем только один бэкап в неделю
                        should_delete = backup_date.weekday() != 0  # Оставляем понедельники
                    elif age < timedelta(days=30 * self.config.KEEP_MONTHLY_BACKUPS):
                        # Оставляем только один бэкап в месяц
                        should_delete = backup_date.day != 1  # Оставляем первое число месяца
                    else:
                        should_delete = True
                ...
                logger.error(f"Error cleaning up backup {backup.get('filename', 'unknown')}: {e}")
        
        # Обновляем метаданные
        self.metadata._metadata["last_cleanup"] = now.isoformat()
        ...
        logger.info(f"Backup cleanup completed: {deleted_count} files deleted ({cleanup_stats['deleted_size_mb']} MB)")
    ...
    async def start_scheduler(self):
        ...
        logger.info(f"Starting backup scheduler: interval {self.config.BACKUP_INTERVAL_HOURS}h")
