...
class RateLimiter:
    ...
    def _setup_default_rules(self):
        ...
        self.rules = {
            ...
            "admin_limit": RateLimitRule(
                requests=2000, # Очень большой лимит для админа
                window=60,
            ),
        }
    ...
    async def check_limit(
        self, request: Request, rule_key: str = "api:general"
    ) -> RateLimitInfo:
        """
        ...
        """
        # ПРОВЕРКА: Если это админ, пропускаем лимиты
        user_info = getattr(request.state, "user", None)
        if user_info and user_info.get("role") in ["admin", "super_admin"]:
            # Возвращаем "бесконечный" лимит
            return RateLimitInfo(
                requests_made=0, requests_remaining=9999, reset_time=time.time() + 3600
            )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "memory_keys": (
                len(self.memory_store._store)
                if hasattr(self.memory_store, "_store")
                else 0
            ),
        }
