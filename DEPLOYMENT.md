...
### Команды быстрого управления (после автоматической установки):
...
├── start.sh                     # Быстрый запуск системы (создается автоматически)
├── stop.sh                      # Быстрая остановка системы (создается автоматически)
├── restart.sh                   # Перезапуск системы (создается автоматически)
├── check-status.sh              # Проверка статуса (создается автоматически)
├── logs.sh                      # Просмотр логов (создается автоматически)
├── backup-system.sh             # Резервное копирование (создается автоматически)
├── update-system.sh             # Обновление системы (создается автоматически)
