...
async def main() -> None:
    ...
    try:
        from utils.system_status import detect_unexpected_shutdown, send_unexpected_shutdown_notification
        crash_info = detect_unexpected_shutdown()
        if crash_info:
            logger.warning("🔴 Бот обнаружил неожиданное завершение системы в предыдущем запуске!")
            ...
            logger.info("✅ Предыдущее завершение бота было нормальным (или первый запуск)")
    except Exception as e:
